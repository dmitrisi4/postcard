	...
	*/ var $event = $.event, $special, resizeTimeout;
    ...
    $.PFold.prototype = {
        ...
        _getTranslationValue: function() {
            var x = 0, y = 0, horizTimes = 0, vertTimes = 0;
        ...
        _getAccumulatedValue: function() {
            var l = 0, r = 0, t = 0, b = 0, horizTimes = 0, vertTimes = 0;
            for(var i = 0; i < this.options.folds; ++i){
                // bottom as default
                var dir = this.options.folddirection[i] || 'bottom';
                switch(dir){
        ...
        _start: function(action, step) {
            ...
            step |= 0;
            var self = this, styleCSS = action === 'fold' ? {
                width: this.lastStyle.width,
                height: this.lastStyle.height,
                left: this.lastStyle.left,
                top: this.lastStyle.top
            } : this.initialDim, contentTopFront = '', contentBottom = '', contentTopBack = '', // direction for step [step]
            // bottom is the default value if none is present
            direction = action === 'fold' ? this.options.folddirection[this.options.folds - 1 - step] || 'bottom' : this.options.folddirection[step] || 'bottom', // future direction value (only for the "fold" action)
            ...
            var unfoldClass = 'uc-unfold-' + direction, topElClasses = action === 'fold' ? 'uc-unfold uc-part ' + unfoldClass : 'uc-part ' + unfoldClass, $topEl = $('<div class="' + topElClasses + '"><div class="uc-front">' + contentTopFront + '</div><div class="uc-back">' + contentTopBack + '</div></div>').css(styleCSS), $bottomEl = $('<div class="uc-part uc-single">' + contentBottom + '</div>').css(styleCSS);
            ...
            // add overlays
            if (this.options.overlays && this.support) this._addOverlays(action, $bottomEl, $topEl);
            setTimeout(function() {
                // apply style
                action === 'fold' ? $topEl.removeClass('uc-unfold') : $topEl.addClass('uc-unfold');
                if (self.support) $topEl.on(self.transEndEventName, function(event) {
                    if (event.target.className !== 'uc-flipoverlay' && step < self.options.folds) // goto next step in [options.folddelay] ms
                    setTimeout(function() {
                ...
                if (self.options.overlays && self.support) {
                    var bo = action === 'fold' ? 1 : 0, tbo = action === 'fold' ? 0.5 : 0, tfo = action === 'fold' ? 0 : 0.5;
        ...
        _getTranslationViewport: function() {
            ...
            if (this.fOffsetL < 0) tx = Math.abs(this.fOffsetL);
            ...
            if (this.options.centered) {
                ...
                if (translationValue.x > 0 && this.fOffsetR + translationValue.x >= 0) ftx = this.fOffsetL >= 0 ? Math.min(translationValue.x, this.fOffsetR) : translationValue.x + (tx - translationValue.x);
    ...
    $.fn.pfold = function(options) {
        ...
        if (typeof options === 'string') {
            ...
            this.each(function() {
                if (!instance) {
                    logError("cannot call methods on pfold prior to initialization; attempted to call method '" + options + "'");
                    return;
                }
...
//# sourceMappingURL=index.274279b1.js.map
